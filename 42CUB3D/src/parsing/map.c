...
/*   Updated: 2026/02/15 13:23:20 by mattcarniel      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	unmap_file(t_file *file)
{
	if (!file || !file->data || file->size <= 0)
		return ;
	munmap((void *)file->data, file->size); //error check ?
	*file = (t_file){0};
}
...
t_file	map_file(const char *path)
{
	...
	if (!path)
		return ((t_file){0}); //error, do we need an access ?
	fd = open(path, O_RDONLY);
	if (fd < 0)
		return ((t_file){0}); //error
	fstat(fd, &st);
	if (st.st_size <= 0)
		return ((t_file){0}); //error
	file.data = (char *)mmap(NULL, st.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
